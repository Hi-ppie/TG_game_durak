...
export function canBeat(attack, defend, trumpSuit) {
...
export function startGame(humanId, botId) {
...
export function ranksOnTable(state) {
    const set = new Set();
    for (const slot of state.table) {
        set.add(slot.attack.rank);
        if (slot.defend)
            set.add(slot.defend.rank);
    }
    return set;
}
// Атаки разрешены в фазах 'attack' и 'throw'
export function legalAttacks(state, playerIndex) {
    if ((state.phase !== 'attack' && state.phase !== 'throw') || playerIndex !== state.attacker)
        return [];
...
export function legalDefenses(state, playerIndex, attackIndex) {
    if (state.phase !== 'defend' || playerIndex !== state.defender)
        return [];
...
export function applyAction(state, playerId, action) {
    ...
    if (me < 0)
        return { ok: false, error: 'Игрок не найден', state };
    if (action.kind === 'attack') {
        if (me !== state.attacker)
            return { ok: false, error: 'Сейчас не ваш ход атаковать', state };
        const legal = legalAttacks(state, me);
        const allowed = legal.some((c) => c.suit === action.card.suit && c.rank === action.card.rank);
        if (!allowed)
            return { ok: false, error: 'Этой картой нельзя атаковать сейчас', state };
        ...
        if (!ok)
            return { ok: false, error: 'Карты нет в руке', state };
    ...
    if (action.kind === 'defend') {
        if (me !== state.defender)
            return { ok: false, error: 'Сейчас не ваш ход защищаться', state };
        ...
        if (!allowed)
            return { ok: false, error: 'Этой картой нельзя побить атаку', state };
        const ok = removeCardFromHand(state.players[me].hand, action.card);
        if (!ok)
            return { ok: false, error: 'Карты нет в руке', state };
    ...
    if (action.kind === 'take') {
        if (me !== state.defender)
            return { ok: false, error: 'Только защищающийся может взять', state };
    ...
    if (action.kind === 'done') {
        if (me !== state.attacker)
            return { ok: false, error: 'Только атакующий завершает ход', state };
        if (state.phase === 'attack') {
            ...
            if (!allDefended)
                return { ok: false, error: 'Есть непобитые атаки. Либо добавьте, либо защитник пусть возьмёт', state };
